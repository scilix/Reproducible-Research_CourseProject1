...
The 5-min-interval at `r hms::as.hms(mean_steps_per_5min$minutes[which.max(mean_steps_per_5min$steps)]*60)` has the highest step count throughout the day (a total of `r max(mean_steps_per_5min$steps)`). 
...
Over a total of `r nrow(steps_per_day_ai)` days, the average number of steps was `r mean_steps_per_day_ai` and the median of steps per day was `r median_steps_per_day_ai`. The valus are different from the above values and represent a more consistent picture. In the analysis above days with missing values were interpreted as "0 steps", which does not seem to be a usefull assumption. Now, for days without data the average number of steps is used, which is, of course, not correct, but a usefull assumption.
...
```{r, steps per day after imputing - histogram, echo=TRUE}
